 ...
 * Lazy Load
 *
 * add support of images lazy loading
 *
 * @category   Wordpress
...
class LazyLoad {
	...
	public function run_lazy_load() {
		...
		if ( ! self::skip() ) {
			...
			add_filter( 'StarterKit/media_picture/source_attributes', [ $this, 'process_source_attributes' ],
				PHP_INT_MAX );
	...
	public function add_image_placeholders( $content ) {
		...
		$content = preg_replace_callback( '#<(img)([^>]+?)(>(.*?)</\\1>|[\/]?>)#si', [ $this, 'process_image' ],
			$content );
	...
	public function load_assets() {
		wp_enqueue_script( 'lazy-load', Utils::getConfigSetting( 'assets_uri' ) . 'js/lazyload.js', [],
			Utils::getConfigSetting( 'cache_time' ), true );
	}
