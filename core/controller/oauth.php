...
class oauth {
	...
	function check_request() {
		...
		if( isset( $_GET['oauth'] ) && in_array( $_GET['oauth'], array_keys( $allowed_methods ) ) ) {

			call_user_func( $allowed_methods[ $_GET['oauth'] ]['auth'] );

		} elseif( isset( $_GET['oauth-callback'] ) && in_array( $_GET['oauth-callback'], array_keys( $allowed_methods ) ) ) {
	...
	**/
	function auth_facebook() {
		...
		$fb = new \Facebook\Facebook([
			'app_id' => \ffblank\helper\utils::get_option( 'facebook_app_id' ),
			'app_secret' => \ffblank\helper\utils::get_option( 'facebook_app_secret' ),
		]);
	...
	**/
	private function _get_google_client() {
		...
		$client->setAuthConfig([
			'client_id' => \ffblank\helper\utils::get_option( 'google_client_id'),
			'client_secret' => \ffblank\helper\utils::get_option( 'google_client_secret')
		]);
	...
	**/
	function auth_twitter() {
		...
		$connection = new \Abraham\TwitterOAuth\TwitterOAuth(
			\ffblank\helper\utils::get_option( 'twitter_consumer_key'),
			\ffblank\helper\utils::get_option( 'twitter_consumer_secret')
		);
		...
		$auth_url = $connection->url( "oauth/authorize", array( "oauth_token" => $temporary_credentials['oauth_token']));
		wp_redirect( $auth_url );
		exit;
	...
	**/
	function callback_twitter() {
		...
		$consumer_key = \ffblank\helper\utils::get_option( 'twitter_consumer_key');
		$consumer_secret = \ffblank\helper\utils::get_option( 'twitter_consumer_secret');
