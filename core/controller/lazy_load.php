...
class lazy_load {
	...
	public function process_image_attributes( $attributes) {
		...
		} elseif ( \ffblank\helper\utils::get_option('lazy_load_get_sizes_with_getimagesize', 1) ) {
	...
	function get_placeholder_image($image_width = 24, $image_height = 24) {
		...
		$placeholder_color = \ffblank\helper\utils::get_option('placeholder_color', '#555');
		...
		$svg = base64_encode(FFBLANK()->view->load('/template-parts/lazy-loading-svg', $data, true));
