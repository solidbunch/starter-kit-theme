...
namespace ttt\shortcodes\posts;
...
function load_posts() {
	...
	$shortcode_atts = \ttt\helper\utils::sanitize_array_text_params( $shortcode_atts );
	...
	$query_vars = \ttt\helper\utils::sanitize_array_text_params( $query_vars );
	
	// query for posts
	$query = TTT()->model->post->get_posts( $query_vars );
	
	// display posts
	while ( $query->have_posts() ): $query->the_post();
		
		TTT()->view->load( '/view/loop_item', array(
			'atts' => $shortcode_atts
